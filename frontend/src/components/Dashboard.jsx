...
const Dashboard = ({ apiUrl }) => {
  ...
  const fetchLeads = async () => {
    try {
      ...
      setError('Failed to fetch leads. Please make sure the backend server is running.');
  ...
  const filteredLeads = leads.filter(lead => {
    const matchesEmployee = !filterEmployee || lead.employeeName === filterEmployee;
  ...
  const getStatusColor = (status) => {
    const colors = {
      ...
      'Converted': 'bg-indigo-100 text-indigo-800 border-indigo-300',
    ...
    <div className="min-h-screen bg-slate-50 flex flex-col">
      ...
      <header className="bg-gradient-to-r from-blue-600 via-purple-600 to-indigo-600 border-b border-slate-200 shadow-sm">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
          <h1 className="text-3xl font-semibold text-white">Leads Dashboard</h1>
          <p className="mt-1 text-sm text-blue-100">View and manage leads generated by your team</p>
        </div>
      </header>
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
          <div className="bg-gray-100 rounded-lg border border-gray-200 shadow-sm p-6 hover:shadow-md transition-shadow">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-3xl font-semibold text-gray-800">{leads.length}</p>
              </div>
          ...
          <div className="bg-gray-100 rounded-lg border border-gray-200 shadow-sm p-6 hover:shadow-md transition-shadow">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-sm font-medium text-gray-600 mb-1">Filtered Leads</p>
                <p className="text-3xl font-semibold bg-gradient-to-r from-blue-600 via-purple-600 to-indigo-600 bg-clip-text text-transparent">{filteredLeads.length}</p>
              </div>
              <div className="p-3 bg-gray-200 rounded-lg">
                <FilterIcon />
              </div>
          ...
          <div className="bg-gray-100 rounded-lg border border-gray-200 shadow-sm p-6 hover:shadow-md transition-shadow">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-3xl font-semibold text-gray-800">{uniqueEmployees.length}</p>
              </div>
              <div className="p-3 bg-gray-200 rounded-lg">
                <UserIcon />
              </div>
        ...
        <div className="bg-gray-50 rounded-lg border border-gray-200 shadow-sm p-6 mb-6">
          <div className="flex items-center mb-4">
            <FilterIcon />
            <h2 className="ml-2 text-lg font-semibold text-slate-900">Filters</h2>
          </div>
        ...
        <div className="bg-gray-50 rounded-lg border border-gray-200 shadow-sm overflow-hidden">
          <div className="px-6 py-4 bg-gradient-to-r from-blue-600 via-purple-600 to-indigo-600 border-b border-gray-200">
            <h2 className="text-lg font-semibold text-white">Leads Overview</h2>
          </div>
                ...
                {filteredLeads.length === 0 ? (
                  <tr>
                    <td colSpan="10" className="px-6 py-12 text-center">
                      <p className="text-gray-500 text-sm">No leads found matching the selected filters.</p>
                    </td>
                  </tr>
                ) : (
                  filteredLeads
                    ...
                    .map((lead, index) => (
                      ...
                      >
                        ...
                        <td className="px-6 py-4 whitespace-nowrap">
                          <div className="flex items-center">
                            <div className={`w-8 h-8 ${getEmployeeAvatarColor(lead.employeeName)} rounded-full flex items-center justify-center mr-3`}>
                              <span className="text-white text-xs font-semibold">
                                {lead.employeeName.split(' ').map(n => n[0]).join('')}
                              </span>
                            </div>
                        ...
                        <td className="px-6 py-4 whitespace-nowrap">
                          <span className={`px-3 py-1 text-xs font-medium rounded-md border ${getStatusColor(lead.status)}`}>
                            {lead.status}
                          </span>
                        </td>
                        <td className="px-6 py-4 whitespace-nowrap">
